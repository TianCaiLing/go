...
func (c *DoLoginActivity) Post() {
	...
	thetime, _ := time.ParseInLocation("2006-1-02 15:04", c.Input().Get("low_time1"), time.Local)
	...
	beego.Debug(thetime, thetime.Unix())
	...
	//Closetime := Opentime + int64(len(cmd.Rewards))*86400
	//cmd.CloseTime = cmd.OpenTime + uint64(len(cmd.Rewards))*86400
	...
	LoginActivityInsert(c.Input().Get("low_time1"), c.Input().Get("close_time1"), time.Unix(time.Now().Unix(), 0).Format("2006-01-02 15:04:05"))
